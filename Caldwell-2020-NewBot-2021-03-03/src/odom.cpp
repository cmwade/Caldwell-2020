...
float rWidth = 7.3429;
float lWidth = 7.3429;
float bLength = 7.3429;

//A simple way to calculate the new Ratio
float wheeldiameter = 3.28;
float inchestodegreesratio = wheeldiameter*pi/360;

//Ratios of Odom Wheels, in Inches/Degrees
float rWheelRatio = -0.028760;
float lWheelRatio = -0.028674; 
float bWheelRatio = -0.028717; 
...
void turnSlide(float endX, float endY, float endRotationDeg, float maxDriveValue, float maxTurnValue, float timeoutMsec, float drivePValue, float turnPValue, float driveDValue, float turnDValue, float driveErrorMargin, float turnErrorMarginDeg) {
  ...
  while( ((reduceAngleMinus180to180(turnError)) > turnErrorMarginDeg || (reduceAngleMinus180to180(turnError)) < -turnErrorMarginDeg || driveError > driveErrorMargin || driveError < -driveErrorMargin) && Brain.timer(timeUnits::msec) < timeoutMsec && 1==1){
...
void odomAlign(bool Xalign, bool Yalign){
  
  updateEncoders();
  updatePosition();
  if(absGlobalX<72 && absGlobalY<72){
